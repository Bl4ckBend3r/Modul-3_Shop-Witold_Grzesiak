...
<header
  className={clsx(
    ...
    "lg:w-[1440px] lg:h-[244px] lg:px-[40px] lg:pt-[40px] lg:pb-[40px]",
    className
  )}
>
  {/* KONTAINER 1360, odstęp 32px między wierszami */}
  <div className="mx-auto flex w-full max-w-[1360px] flex-col gap-6 md:gap-8 pb-6 lg:pb-[40px]">
    {/* GÓRNY PASEK 54px */}
    <div className="flex  items-center pt-[40px] pb-[40px] justify-between gap-4 md:gap-6 lg:h-[54px] pb-6 lg:pb-[40px]">
      {/* Logo 159x44 */}
      <div
        className="flex h-[44px] w-[159px] items-center font-semibold leading-[400px]"
        style={{ fontFamily: "Inter", fontSize: 32, letterSpacing: "-0.01em" }}
      >
        <span className="text-[#EE701D]">Nexus</span>
        <span className="text-[#262626]">Hub</span>
      </div>
      ...
      {!loggedIn ? (
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          <IconBtn aria-label="Messages" className="hidden sm:inline-flex">
            <MessageSquare className="h-6 w-6 text-[#6B7280]" />
          </IconBtn>
          <IconBtn aria-label="Notifications" className="hidden sm:inline-flex">
  ...
  return (
    <Button
      ...
      className={clsx(
        ...
        active ? "text-[#EE701D] font-semibold" : "text-[#737373] font-medium"
      )}
